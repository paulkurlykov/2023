...
const classFilter = document.querySelectorAll('.like:not(.subscribe)'); // Тоже самое, но можно отфильтровать слово в классе. Возвращает массив Nodelist c подходящими узлами. 
...
const queryAllElem = document.querySelectorAll('.like'); // возвращает массив NodeList с элементами селектора
...
const children = document.querySelector('body').children; // показывает массив HTML Collection с дочерними элементами (одного уровня, вложенных элементов не будет)
...
const getAtt = document.querySelector('[data-say-hi]');
let jonchea;


console.log(getAtt.getAttribute('data-say-hi'));
...
//     if(allElements[i].textContent == 'Йончи') {
//         jonchea = allElements[i];
//     }
// }

for( let elem of allElements) {
    if(elem.textContent == 'Йончи') {
        jonchea = elem;
    }
}
